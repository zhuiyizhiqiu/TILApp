...
struct WebsiteController: RouteCollection {
    ...
    func acronymHandler(_ req: Request) throws -> Future<View>{
        return try req.parameters.next(Acronym.self).flatMap(to: View.self){
            acronym in
            return acronym.user.get(on: req).flatMap(to: View.self){
                user in
    ...
    func allUsersHandler(_ req:Request) throws -> Future<View>{
        return User.query(on: req).all().flatMap(to: View.self){
            users in
            let context = AllUserContext(title: "All User", users: users)
    ...
    func editAcronymHandler(_ req: Request) throws -> Future<View>{
        return try req.parameters.next(Acronym.self).flatMap(to: View.self){
            ...
            let categories = try acronym.categories.query(on: req).all()
            let context = EditAcronymContext(acronym: acronym, users: user, categories: categories)
            return try req.view().render("createAcronym", context)
        }
    }
    ...
    func editAcronymPostHandler(_ req: Request)throws -> Future<Response>{
        return try flatMap(to: Response.self, req.parameters.next(Acronym.self), req.content.decode(CreateAcronymData.self)){
            ...
            return acronym.save(on: req).flatMap(to: [Category].self){
                _ in
                try acronym.categories.query(on: req).all()
                }.flatMap(to: Response.self){
                    exisitingCategories in
                    let existingStringArray = exisitingCategories.map{
                        $0.name
                    }
                    ...
                    for categoryNameToRemove in categoriesToRemove{
                        let categortToRemove = exisitingCategories.first{
                            $0.name == categoryNameToRemove
                        }
                        if let category = categortToRemove {
